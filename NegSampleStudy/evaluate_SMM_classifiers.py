...
if __name__ == "__main__":
    ...
    modpath = osp.join("/data/mdrnevich/ml4nw/NegSampleStudy/models/classifier_SMMr_batch{}_sample{}.zip".format(batch_size, sample_ix))
    mc_model = utils.models.load_model(modpath, device=DEVICE).to(DEVICE)
    mc_X_scaler, mc_weight_norm = utils.preprocessing.load_scaling(modpath)
    ...
    rval = np.array((sample_ix,
                     ...
                     test_loss,
                     divergence))
    np.save("/data/mdrnevich/ml4nw/NegSampleStudy/temp/results_SMMr_{}.npy".format(sample_ix), rval)
